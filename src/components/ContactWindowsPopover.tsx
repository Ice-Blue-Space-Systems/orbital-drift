...
const ContactWindowsPopover: React.FC<ContactWindowsPopoverProps> = ({
  ...
  const getStatusIcon = () => {
    if (activeWindow) return <SignalCellularAltIcon sx={{ color: "#00ff41" }} />;
    return <EventIcon />;
  };
      ...
      <Tooltip 
        title={
          activeWindow 
            ...
            : upcomingWindows > 0 
              ? `${upcomingWindows} Upcoming Contact${upcomingWindows !== 1 ? 's' : ''}` 
              : "No Upcoming Contacts"
        }
        arrow
      >
        <Badge 
          badgeContent={activeWindow ? "LIVE" : upcomingWindows > 0 ? upcomingWindows : 0}
          color={activeWindow ? "success" : "warning"}
          variant={activeWindow ? "standard" : "dot"}
          invisible={!activeWindow && upcomingWindows === 0}
        >
          <IconButton
            onClick={handleClick}
            sx={{
